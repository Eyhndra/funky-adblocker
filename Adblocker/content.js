...
function adFunction() {
    ...
    const slotAd = document.getElementsByClassName(
        "style-scope ytd-ad-slot-renderer"
    );

    const engagementAd = document.getElementsByClassName(
    ...
    while (slotAd.length > 0 ){
        slotAd[0].parentNode.removeChild(slotAd[0]);
    }
    while (engagementAd.length > 0 ){
        engagementAd[0].parentNode.removeChild(engagementAd[0]);
    }
